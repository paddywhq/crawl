...
const patternForXbox360Date = `<li><label>Original release date\s*:</label>\s*(\d+/\d+/\d+)\s*</li>`
const patternForXbox360Developer = `<li><label>Developer\s*:</label>\s*(.*?)\s*</li>`
const patternForXbox360Publisher = `<li><label>Publisher\s*:</label>\s*(.*?)\s*</li>`
const patternForXbox360Genre = `<li><label>Genre\s*:</label>\s*(.*?)\s*</li>`
...
const patternForXbox1Detail = `<label>.*?description:</label><span>([\s\S]*?)</span>`
const patternForXbox1Date = `<label>Release date:</label>\s*<span>(\d+/\d+/\d+)</span>`
const patternForXbox1Developer = `<label>Developer:</label><span>([\s\S]*?)</span>`
const patternForXbox1Publisher = `<label>Publisher:</label><span>([\s\S]*?)</span>`
const patternForXbox1Genre = `<label>Genre:</label><span>([\s\S]*?)</span>`
	...
	for _, f := range files{
		...
		if strings.Contains(f, "360_") {
			...
			for _, game := range result{
				...
				url := game[1]
				if strings.Contains(url, "/en-US/Xbox-One/Bundle/") {
					continue
				}
